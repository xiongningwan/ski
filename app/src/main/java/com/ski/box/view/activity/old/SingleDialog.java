...
public class SingleDialog {
    ...
    private final ParamsAdapter mAdapter;
    RecyclerView rv;

    public SingleDialog(@NonNull Context context, List<MerchantBean> list) {
        mContext = context;
...
//        mAdapter.setNewData(list);
    ...
    public static class ParamsAdapter extends BaseQuickAdapter {
        ...
        public ParamsAdapter(List<MerchantBean> list) {
            super(R.layout.ski_dialog_single_item, list);
        }

        @Override
        protected void convert(@NotNull BaseViewHolder holder, @Nullable Object o) {
            MerchantBean bean = (MerchantBean) o;
            holder.setText(R.id.tv_name, bean.getMerchantName());
        }
    }
}
