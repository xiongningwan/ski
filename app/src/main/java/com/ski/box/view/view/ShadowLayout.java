...
public class ShadowLayout extends FrameLayout {
    ...
    private boolean shadowArc;
    private float startAngle=-134;
    private float sweepAngle=85;

    private final float density = AppUtil.getContext().getResources().getDisplayMetrics().density;
    ...
    private void initAttributes(AttributeSet attrs) {
        TypedArray attr = getContext().obtainStyledAttributes(attrs, R.styleable.ski_ShadowLayout);
        if (attr == null) {
            return;
        }
        ...
        try {
            //默认是显示
            leftShow = attr.getBoolean(R.styleable.ski_ShadowLayout_ski_hl_leftShow, true);
            rightShow = attr.getBoolean(R.styleable.ski_ShadowLayout_ski_hl_rightShow, true);
            bottomShow = attr.getBoolean(R.styleable.ski_ShadowLayout_ski_hl_bottomShow, true);
            topShow = attr.getBoolean(R.styleable.ski_ShadowLayout_ski_hl_topShow, true);
            mCornerRadius = attr.getDimension(R.styleable.ski_ShadowLayout_ski_hl_cornerRadius, getResources().getDimension(R.dimen.ski_dp_0));
            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.ski_ShadowLayout_ski_hl_shadowLimit, getResources().getDimension(R.dimen.ski_dp_5));

            //x轴偏移量
            mDx = attr.getDimension(R.styleable.ski_ShadowLayout_ski_hl_dx, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.ski_ShadowLayout_ski_hl_dy, 0);
            mShadowColor = attr.getColor(R.styleable.ski_ShadowLayout_ski_hl_shadowColor, ContextCompat.getColor(getContext(),R.color.ski_default_shadow_color));
            //判断传入的颜色值是否有透明度
            isAddAlpha(mShadowColor);
            mBackGroundColor = attr.getColor(R.styleable.ski_ShadowLayout_ski_hl_shadowBackColor,  ContextCompat.getColor(getContext(),R.color.ski_default_shadowback_color));

            shadowArc = attr.getBoolean(R.styleable.ski_ShadowLayout_ski_hl_shadowArc, false);
            startAngle = attr.getDimension(R.styleable.ski_ShadowLayout_ski_hl_startAngle, 0);
            sweepAngle = attr.getDimension(R.styleable.ski_ShadowLayout_ski_hl_sweepAngle, 0);
        } finally {
            attr.recycle();
        }
    }
    ...
    private Bitmap createShadowBitmap(int shadowWidth, int shadowHeight, float cornerRadius, float shadowRadius,
        ...
        /*特殊画圆弧*/
        if (shadowArc) {
            canvas.drawArc(shadowRect, startAngle/density, sweepAngle/density, false, shadowPaint);
        } else {
