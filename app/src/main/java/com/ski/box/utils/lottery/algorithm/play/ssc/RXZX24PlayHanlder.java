 ...
 * 任四组选24 1150201
 */
public class RXZX24PlayHanlder implements ITicketPlayHandler {

    private int rxNum;

    public RXZX24PlayHanlder( int rxNum) {
        this.rxNum = rxNum;
    }
            ...
            if (data.length != 2) {
                ...
                if (offsets.length == fixedOffsets.length && offsets.length >= this.rxNum && offsets.length <= OFFSETS_WUXIN.length) {
                    ...
                    if (!data[0].trim().equals(ofsb.substring(0, ofsb.length() - 1))) {
                        ...
                        if (nums.length == fixedNums.length && nums.length >= 4) {
    ...
    public int calculateBetNum(String betNums) {
        ...
        if (data.length != 2) {
            ...
            String[] offsets = data[0].trim().split(",");
            String[] nums = data[1].trim().split(" ");
            return nums.length < 4 ? 0 : SIXING_MULTI[offsets.length - this.rxNum] * nums.length * (nums.length - 1) * (nums.length - 2) * (nums.length - 3) / 24;
        }
    }
}
