 ...
 * 任三组选六复式 1140202
 */
public class RXZXZ6PlayHanlder implements ITicketPlayHandler {

    private int rxNum;

    public RXZXZ6PlayHanlder( int rxNum) {
        this.rxNum = rxNum;
    }
            ...
            if (data.length != 2) {
                ...
                if (offsets.length == fixedOffsets.length && offsets.length >= this.rxNum) {
                    ...
                    if (!data[0].trim().equals(ofsb.substring(0, ofsb.length() - 1))) {
                        ...
                        if (nums.length == fixedNums.length && fixedNums.length >= 3 && fixedNums.length <= 10) {
    ...
    public int calculateBetNum(String betNums) {
        ...
        if (data.length != 2) {
            ...
            String[] nums = data[1].trim().split(" ");
            if (this.rxNum == 3) {
                return nums.length * (nums.length - 1) * (nums.length - 2) * SANXING_MULTI[offsets.length - this.rxNum] / 6;
            } else {
                if (this.rxNum == 2) {
                }
