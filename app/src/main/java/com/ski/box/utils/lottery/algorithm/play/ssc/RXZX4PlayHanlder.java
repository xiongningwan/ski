 ...
 * 任四组选4  1150204
 */
public class RXZX4PlayHanlder implements ITicketPlayHandler {
    private int rxNum;

    public RXZX4PlayHanlder(int rxNum) {
        this.rxNum = rxNum;
    }
    ...
    public int calculateBetNum(String betNums) {
        ...
        if (data.length != 2) {
            ...
            if (nums.length != 2) {
                ...
                return SIXING_MULTI[offsets.length - this.rxNum] * (fewerNums.length * (moreNums.length - in) + (fewerNums.length - 1) * in);
            }
        }
    }
}
