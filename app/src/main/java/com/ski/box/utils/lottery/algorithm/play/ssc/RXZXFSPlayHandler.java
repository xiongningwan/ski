 ...
 * 任二组选复式 1120201
 */
public class RXZXFSPlayHandler implements ITicketPlayHandler {
    private int rxNum;

    public RXZXFSPlayHandler( int rxNum) {
        this.rxNum = rxNum;
    }
                ...
                if (offsets.length == fixedOffsets.length && offsets.length >= this.rxNum) {
                    ...
                    if (!data[0].trim().equals(ofsb.substring(0, ofsb.length() - 1))) {
                        ...
                        if (nums.length != fixedNums.length) {
                            ...
                            return offsets.length >= this.rxNum && nums.length >= this.rxNum;
                        }
                    }
                } else {
                    return false;
    ...
    public int calculateBetNum(String betNums) {
        ...
        if (data.length != 2) {
            ...
            int a;
            for(a = 0; a < this.rxNum; ++a) {
                upCount *= nums.length - a;
            }

            for(a = 1; a <= this.rxNum; ++a) {
                downCount *= a;
            }

            return ERXING_MULTI[offsets.length - this.rxNum] * upCount / downCount;
        }
    }
}
