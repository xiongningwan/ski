...
public class GroupRecordBetPresenter extends RxPresenter<GroupRecordBetContract.View> implements GroupRecordBetContract.Presenter {
    private IGroupModel mGroupModel;

    public GroupRecordBetPresenter(Context context) {
        super(context);
        mGroupModel = new GroupModel();
    }

    @Override
    public void getTeamOrderList(String startDate, String endDate, String ticketId, String status, String memberAccount, int pageSize, int pageNum) {
        Disposable disposable = mGroupModel.getTeamOrderList(new Consumer<GroupBetData>() {
            @Override
            public void accept(GroupBetData o) throws Exception {
        ...
        }, startDate, endDate, ticketId, status, memberAccount, pageSize, pageNum);
