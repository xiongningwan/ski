...
public class NotchHelper {
    ...
    private static Rect get3rdSafeInsetRect(Context context){
        // 全面屏设置项更改
        if (SystemUtils.isHuawei()) {
            boolean isHuaweiNotchSetToShow = SystemUtils.huaweiIsNotchSetToShowInSetting(context);
    ...
    private static Rect getRectInfoRotation0(Context context) {
        ...
        if (SystemUtils.isVivo()) {
            ...
            rect.top = StatusBarUtil.getStatusBarHeight((Activity) context);
            rect.bottom = 0;
    ...
    private static Rect getRectInfoRotation90(Context context) {
        ...
        if (SystemUtils.isVivo()) {
            ...
            rect.left = StatusBarUtil.getStatusBarHeight((Activity) context);
            rect.right = 0;
        } else if (SystemUtils.isHuawei()) {
    ...
    private static Rect getRectInfoRotation180(Context context) {
        ...
        if (SystemUtils.isVivo()) {
            ...
            rect.top = 0;
            rect.bottom = StatusBarUtil.getStatusBarHeight((Activity) context);
        } else if (SystemUtils.isHuawei()) {
            int[] notchSize = getNotchSizeInHuawei(context);
            rect.top = 0;
    ...
    private static Rect getRectInfoRotation270(Context context) {
        ...
        if (SystemUtils.isVivo()) {
            ...
            rect.right = StatusBarUtil.getStatusBarHeight((Activity) context);
            rect.left = 0;
        } else if (SystemUtils.isHuawei()) {
            if (sHuaweiIsNotchSetToShow) {
    ...
    public static int getNotchHeightInXiaomi(Context context) {
        ...
        return StatusBarUtil.getStatusBarHeight((Activity) context);
    }

    public static int getNotchWidthInVivo(Context context){
        return StatusBarUtil.dp2px(context, 100);
    }

    public static int getNotchHeightInVivo(Context context){
        return StatusBarUtil.dp2px(context, 27);
    }
    ...
    public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view){
        return SystemUtils.isXiaomi() && hasNotch(view);
    }
}
